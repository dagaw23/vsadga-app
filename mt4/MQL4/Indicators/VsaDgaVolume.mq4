...
//--- plot Less Volume Then 2 Previous Volume Bars
#property indicator_label3  "LessThenPrev2Volume"
#property indicator_type3   DRAW_HISTOGRAM
#property indicator_color3  clrMagenta
#property indicator_style3  STYLE_SOLID
#property indicator_width3  3
//--- plot Not End Bar Volume and Level Bar Volume
#property indicator_label4  "NotEndBarVolume"
...
double volumeLess2Buffer[];
double volumeLevelBuffer[];

double prevBarClose = 0;
int prevBarVolume = 0;
int prevPrevBarVolume = 0;

string zero_date = "1970.01.01%2000:00";
string access_http = "http://generatedata.biz/mt4/smart.php";
...
void updateIndicatorBuffers(int barShift, int barVolume) {
   ...
   if (iClose(NULL, 0, barShift) > prevBarClose) {
	  ...
	  if (barVolume < prevBarVolume && prevBarVolume < prevPrevBarVolume) {
         volumeLess2Buffer[barShift] = barVolume;
	  } else {
		volumeUpBuffer[barShift] = barVolume;
	  }
      ...
      if (barVolume < prevBarVolume && prevBarVolume < prevPrevBarVolume) {
		...
		volumeDownBuffer[barShift] = barVolume;
	  }
      ...
      if (barVolume < prevBarVolume && prevBarVolume < prevPrevBarVolume) {
		...
		volumeLevelBuffer[barShift] = barVolume;
	  }

   }
   
   // aktualizacja zamkniecia bara i wolumenow:
   prevBarClose = iClose(NULL, 0, barShift);
   prevPrevBarVolume = prevBarVolume;
   prevBarVolume = barVolume;
}
...
void writeFreshVolumeData(string data) {
   ...
   for (int i=iSplit-1; i>0; i--) {
      ...
      if (bar_shift == 0) {
         ...
         volumeLevelBuffer[bar_shift] = bar_volume;
      
         Log("Equal Fresh:" + bar_date + "," + barNotEndDatetime + ", bar_shift=" + bar_shift 
            + ", not end:" + volumeLevelBuffer[bar_shift] + ", up:" + volumeUpBuffer[bar_shift] + ", down:" + volumeDownBuffer[bar_shift] + ".");
      } else {
...
void writeUpdatedVolumeData(string data, int ratesTotal) {
   ...
   for (int i=iSplit-1; i>0; i--) {
      ...
      if (bar_date == barNotEndDatetime && bar_shift == 1) {
         // usuniecie poprzedniego:
         volumeLevelBuffer[bar_shift] = EMPTY_VALUE;
...
{
   if (StringSubstr(fxSymbol, 0, 6) == "AUDUSD")
      return "6a";
   else if (StringSubstr(fxSymbol, 0, 6) == "GBPUSD")
      return "6b";
   else if (StringSubstr(fxSymbol, 0, 6) == "USDCAD")
      return "6c";
   else if (StringSubstr(fxSymbol, 0, 6) == "EURUSD")
      return "6e";
   else if (StringSubstr(fxSymbol, 0, 6) == "USDJPY")
      return "6j";
   else if (StringSubstr(fxSymbol, 0, 6) == "NZDUSD")
      return "6n";
   else if (StringSubstr(fxSymbol, 0, 6) == "USDCHF")
      return "6s";
   else if (StringSubstr(fxSymbol, 0, 4) == "GOLD")
      return "gc";
   else if (StringSubstr(fxSymbol, 0, 3) == "OIL")
      return "cl";
   else if (StringSubstr(fxSymbol, 0, 6) == "Copper")
      return "hg";
   else if (fxSymbol == "Natural Gas")
      return "ng";
   else if (StringSubstr(fxSymbol, 0, 6) == "SILVER")
      return "si";
   else if (StringSubstr(fxSymbol, 0, 5) == "Wheat")
      return "zw";
      ...
      return "zb";
   else if (StringSubstr(fxSymbol, 0, 5) == "GER30")
      return "fdax";
...
string getAllBySmartUrl() {
   string result_url = StringConcatenate(access_http,
      "?symbol=", Symbol(),
      "&timeframe=",Period(),
...
void indicatorInit() {
   IndicatorShortName("VsaDga Volume Indicator");
   ...
   SetIndexBuffer(2, volumeLess2Buffer);
   SetIndexStyle(3, DRAW_HISTOGRAM);
   SetIndexBuffer(3, volumeLevelBuffer);
}
