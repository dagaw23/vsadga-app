...
public class ChartWriterImpl implements ChartWriter {
	...
	@Override
	public boolean deleteAccumulateChartJpg(CurrencySymbol symbol, TimeFrame timeFrame)
	...
	public boolean deleteChartJpg(CurrencySymbol symbol, TimeFrame timeFrame) throws BaseServiceException {
		File file = getChartFile(symbol, timeFrame.getTimeFrameDesc());
	...
	public void initConfigParams(String pathToJasperFile, String pathToJpgFile) throws BaseServiceException {
		LOGGER.info("   [init]:" + pathToJasperFile + "," + pathToJpgFile + ".");
	...
	public boolean isNotInitConfig() {
		if (this.pathToJasperFile == null || this.pathToJasperFile.trim().isEmpty())
			return true;
		
		if (this.pathToJpgFile == null || this.pathToJpgFile.trim().isEmpty())
			return true;
	...
	public ByteArrayOutputStream writeChartToJpg(CurrencySymbol symbol, String timeFrameDesc, int barCount) throws BaseServiceException {
		try {
			// utwórz wykres:
			JFreeChart chart = createCombinedChart(symbol, timeFrameDesc, barCount);
			
			// plik wykresu:
			//File chart_file = getTempChartFile(symbol, timeFrameDesc);

			// zapisz wykres do JPG:
			//ChartUtilities.saveChartAsJPEG(chart_file, chart, CHART_WIDTH, CHART_HEIGHT);
			ByteArrayOutputStream out = new ByteArrayOutputStream();
			ChartUtilities.writeChartAsPNG(out, chart, 850, 850);
	...
	public void writeChartToJpg(CurrencySymbol symbol, TimeFrame timeFrame) throws BaseServiceException {
		...
		try {
			...
			// utwórz wykres:
			JFreeChart chart = createCombinedChart(symbol, timeFrame.getTimeFrameDesc(), bar_count);

			// zapisz wykres do JPG:
			ChartUtilities.saveChartAsJPEG(getChartFile(symbol, timeFrame.getTimeFrameDesc()), chart, CHART_WIDTH, CHART_HEIGHT);
	...
	private JFreeChart chartMe(OHLCDataset barDataSet, CurrencySymbol symbol, String timeFrameDesc) {
		// plot:
		CombinedDomainXYPlot combined_plot = getCombinedPlot(barDataSet);
		
		// utworzenie wykresu:
		JFreeChart chart = new JFreeChart(getChartName(symbol, timeFrameDesc), null, combined_plot, false);
		chart.setBackgroundPaint(Color.white);
		
		return chart;
	}
	
	private JFreeChart createCombinedAccumulateChart(CurrencySymbol symbol, TimeFrame timeFrame,
			int barToPrintCount) throws BaseServiceException {
		// wczytaj dane z tabeli:
		OHLCDataset bar_dataset = getDataset(symbol, timeFrame.getTimeFrameDesc(), barToPrintCount);
		...
		// ---
		XYPlot absorbVolumePlot = new XYPlot(getVolumeDataset(abs_volume_dataset, 10 * 1000), domainAxis,
				accuVolumeAxis, absVolumeRenderer);
		...
		JFreeChart chart = new JFreeChart(getChartName(symbol, timeFrame.getTimeFrameDesc()), null, combinedPlot, false);
		chart.setBackgroundPaint(Color.white);

		return chart;
	}

	private JFreeChart createCombinedChart(CurrencySymbol symbol, String timeFrameDesc, int barToPrintCount)
			throws BaseServiceException {
		// wczytaj dane z tabeli:
		OHLCDataset bar_dataset = getDataset(symbol, timeFrameDesc, barToPrintCount);

		return chartMe(bar_dataset, symbol, timeFrameDesc);
	}

	private OHLCDataset getAbsorbVolumeDataset(CurrencySymbol symbol, TimeFrame timeFrame, int barToPrintCount)
			throws BaseServiceException {
	...
	private String getAccumPdfFileName(String symbolName1, String symbolName2) {
		return "ACC_VOL_" + symbolName1 + "_" + symbolName2 + "_"
				+ DateConverter.dateToString(new Date(), "yyMMdd-HHmm") + ".pdf";
	...
	private File getTempChartFile(CurrencySymbol symbol, String timeFrameDesc) {
		return new File(pathToJpgFile + getTempChartFileName(symbol.getSymbolName(), timeFrameDesc));
	}
	
	private File getChartFile(CurrencySymbol symbol, String timeFrameDesc) {
		return new File(pathToJpgFile + getChartFileName(symbol.getSymbolName(), timeFrameDesc));
	}
	...
	private String getTempChartFileName(String symbolName, String timeFrameDesc) {
		return symbolName + "-" + timeFrameDesc + "-temp.jpg";
	}

	private String getChartName(CurrencySymbol symbol, String timeFrameDesc) {
		return "Wykres " + symbol.getSymbolName() + " - " + timeFrameDesc;
	}

	private CombinedDomainXYPlot getCombinedPlot(OHLCDataset barDataSet) {
		// 1) Axis:
		DateAxis domainAxis = new DateAxis("Czas");
		domainAxis.setLowerMargin(0.02); // reduce the default margins
		domainAxis.setUpperMargin(0.02);
		...
		// 3) Main & volume plot:
		XYPlot mainPlot = new XYPlot(barDataSet, domainAxis, rangeAxis, chartRenderer);
		...
		// ---
		XYPlot volumePlot = new XYPlot(getVolumeDataset(barDataSet, 10 * 1000), domainAxis, volumeAxis,
				volumeRenderer);
		volumePlot.setBackgroundPaint(Color.white);
	...
	private OHLCDataset getDataset(CurrencySymbol symbol, String timeFrameDesc, int barToPrintCount)
			throws BaseServiceException {
		BarData bar_data = null;
		BigDecimal prev_bar_close = null;
		LOGGER.info("   [getDataset] " + symbol.getSymbolName() + "," + timeFrameDesc + ".");
		...
		// pobierz dane z tabeli:
		List<BarData> barData_list = currencyDataService.getLastBarDataSortAsc(symbol.getId(), timeFrameDesc,
				barToPrintCount);
		...
		for (int i = 0; i < barData_list.size(); i++) {
			...
			// kolor dla bara danych:
			if (bar_data.getProcessPhase().intValue() < 1) {
				barSeriesColor.add(Color.DARK_GRAY);
			} else {
