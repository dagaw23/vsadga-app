...
public class ReportPrinterBatch extends BaseBatch {
	...
	public void cronJob() {
		...
		try {
			...
			if (symbol_list.isEmpty() || tmefrm_list.isEmpty()) {
				LOGGER.info("   [BATCH] Symbole nie sa aktywne [" + symbol_list.size()
						+ "] lub ramy czasowe [" + tmefrm_list.size() + "].");
				return;
			}
	...
	private boolean isProcessBatch() throws BaseServiceException {
		...
		if (is_report == null)
			throw new BaseServiceException("::isProcessBatch:: brak parametru IS_BATCH_REPORT_PRINTER [" + is_report
					+ "] w tabeli parametrow.");
